...
export default class extends Stage {
    protected *G(): Generator<void, void, unknown> {
        ...
        yield* this.text("「ほう。」", { name: "シュンスケ" })
        yield* this.text("「相手もそう思ってるっぽかったですよ。<br>『SILOは合成人を絶滅させる手段に成り得る』と。」", { name: "マス" })
        yield* this.text("「……。」", { name: "シュンスケ" })
        yield* this.text("「まあ当局からしたら都合は良いですもんね。大戦の遺産を消せるなら。」", { name: "マス" })
        yield* this.text("「だが奴らはアイツらとも手を組んだ。」", { name: "シュンスケ" })
        yield* this.text("「『日本を取り戻す』とかいう奴ですか。名前に反してアナキストの。」", { name: "マス" })
        yield* this.text("「<ruby>あの合成人<rt>ササキ</rt></ruby>が先制攻撃なんかしないだろ。奴らが世論に耐えられなくなってる証拠だ。」", { name: "シュンスケ" })
        yield* this.text("「あんたも煽るのは程々にしてくれよ。」", { name: "シュンスケ" })
    }
}
